...
## Data Flow in Single Perceptron

![single-neuron](/assets/img/single-neuron.png)

Taking just one example input and a neuron of the first layer, `z1`  is the
mathematical operation that is happening during the forward propagation in the 
network. In order to get this operation we perform the matrix multiplication with NumPy
with `@` operator on the arrays. We use [ReLU](https://en.wikipedia.org/wiki/Rectified_linear_unit)
as an activation function in the first layer. Activation functions are important 
neural networks because without then it only becomes a linear regression.

![relu-plot](/assets/img/relu-plot.png)

$$

{\displaystyle \operatorname {ReLU} (x)=x^{+}=f(x)=\max(0,x)={\frac {x+|x|}{2}}={\begin{cases}x&{\text{if }}x>0,\\0&x\leq 0\end{cases}}}
...
We will be need the derivative of ReLU during the backpropagation so lets look at
their implementation and plot too.
